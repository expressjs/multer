...
`originalname` | اسم الملف على كمبيوتر المستخدم، أو المسار الكامل عند تفعيل `preservePath: true` |
...
عند تفعيل `preservePath`، يمرر Multer اسم الملف الوارد مع أي مقاطع مسار يقدمها العميل. يتم كشف ذلك في `file.originalname`؛ ولا يغيّر مجلد الوجهة، ولا ينشئ أدلة، ولا ينظّف المسار نيابةً عنك. `file.originalname` هو دائمًا قيمة مقدّمة من العميل ويجب التعامل معه على أنه غير موثوق؛ مع `preservePath` يحتوي أيضًا على مقاطع المسار التي أرسلها العميل. قم بتطبيعه (normalize) أو التحقق منه (validate) قبل استخدامه في `filename` مخصص أو محرك تخزين.
