...
Multer — это middleware для фреймворка express для обработки`multipart/form-data`, нужная в первую очередь при загрузке файлов. Написана как обертка над [busboy](https://github.com/mscdex/busboy) для максимально эффективного использования.
...
Multer добавляет объект `body` и объект `file` или `files` внутрь объекта `request`. Объект `body` содержит значения текстовых полей формы, объект `file` или `files` содержит файлы, загружаемые через форму.
...
Multer принимает объект с опциями. Базовая опция `dest` указывает Multer, куда загружать файлы. Если вы опускаете объект с опциями, файлы будут находиться в памяти и не будут записаны на диск.

По умолчанию, Multer переименовывает файлы, чтобы избежать конфликтов. В зависимости от ваших потребностей, переименование может быть кастомизировано.
...
`destination` используется, чтобы задать каталог, в котором будуд размещены файлы. Может быть задан строкой (например, `'/tmp/uploads'`). Если не задано расположение `destination`, операционная система использует для сохранения каталог для временных файлов.
...
имя файла `filename` не задано, каждому файлу будет сконфигурировано случайное имя без файлового расширения.

**Важно:** Multer не добавляет никакого файлового расширения, ваша функция должна возвращать имя файла с расширением.
