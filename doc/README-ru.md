...
`originalname` | Имя файла на компьютере пользователя или полный путь, когда включён `preservePath: true` |
...
Когда `preservePath` включён, Multer передаёт входящее имя файла вместе с сегментами пути, предоставленными клиентом. Это значение доступно как `file.originalname`; оно не изменяет целевую папку, не создаёт каталоги и не очищает путь за вас. `file.originalname` всегда предоставляется клиентом и должно рассматриваться как ненадёжное; при `preservePath` оно также содержит сегменты пути, отправленные клиентом. Нормализуйте или проверяйте его перед использованием в собственном `filename` или движке хранения.
