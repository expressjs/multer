...
import { pipeline as _pipeline, PassThrough } from 'node:stream'
...
function collectFiles (busboy, limits, fileFilter, disableDetection) {
  return new Promise((resolve, reject) => {
    const result = []
    ...
    busboy.on('file', async (fieldname, fileStream, filename, encoding, mimetype) => {
      ...
      const detector = disableDetection ? new PassThrough() : new FileType()
      ...
      const promise = pipeline(fileStream, detector, target)
        .then(async () => {
          ...
          const fileType = disableDetection ? null : await detector.fileTypePromise()
...
export default async function readBody (req, limits, fileFilter, disableDetection) {
  const busboy = new Busboy({ headers: req.headers, limits: limits })

  const fields = collectFields(busboy, limits)
  const files = collectFiles(busboy, limits, fileFilter, disableDetection)
