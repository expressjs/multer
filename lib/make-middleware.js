...
// busboy's own default for limits.fieldSize. fileSize and parts default to
// Infinity, but this one is finite, so the "max allowed" adjustment below has
// to be made even when the caller passed no limits at all.
var DEFAULT_FIELD_SIZE = 1024 * 1024
...
function makeMiddleware (setup) {
  return function multerMiddleware (req, res, next) {
    ...
    // busboy fires 'limit' when the file size reaches limits.fileSize, fires
    // 'partsLimit' when the part count reaches limits.parts, and marks a field
    // value truncated when it reaches limits.fieldSize. Multer treats all three
    // as "max allowed", so pass one more to busboy and let it report only once
    // the limit is actually exceeded. fieldSize is adjusted even when the
    // caller set no limits, because busboy defaults it to a finite 1MB.
    var fieldSize = limits && typeof limits.fieldSize === 'number'
      ? limits.fieldSize
      : DEFAULT_FIELD_SIZE

    if (isFinite(fieldSize)) {
      busboyLimits.fieldSize = fieldSize + 1
    }

    if (limits && typeof limits.fileSize === 'number' && isFinite(limits.fileSize)) {
      busboyLimits.fileSize = limits.fileSize + 1
    }

    if (limits && typeof limits.parts === 'number' && isFinite(limits.parts)) {
    ...
    busboy.on('file', function (fieldname, fileStream, { filename, encoding, mimeType }) {
      ...
      // Reverse the WHATWG escapes once, before anything can build an error
      // about this file, so err.filename and file.originalname are the same
      // string. busboy reports filename as undefined for a part that carries
      // no file, and such a part never reaches an error that names a file.
